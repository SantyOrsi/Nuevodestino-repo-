...
class QuoteForm {
  constructor(formSelector) {
    ...
    this._movDetalleEl    = document.getElementById('js-movimiento-detalle');
    this._movDetalleInput = document.getElementById('q-mov-detalle');

    this._bindSubmit();
    this._bindMovimiento();
  }

  /**
   * Muestra u oculta el campo de detalle de movimiento
   * según la opción Sí / No seleccionada.
   */
  _bindMovimiento() {
    const radios = this._form.querySelectorAll('input[name="q-movimiento"]');
    if (!radios.length || !this._movDetalleEl) return;

    radios.forEach(radio => {
      radio.addEventListener('change', () => {
        const esSi = radio.value === 'si' && radio.checked;
        this._movDetalleEl.hidden = !esSi;

        /* Si se oculta, limpiar el campo para no enviar datos residuales */
        if (!esSi && this._movDetalleInput) {
          this._movDetalleInput.value = '';
        }
      });
    });
  }
