...
/* ── Campo condicional de movimiento en el formulario de contacto ── */
(function () {
  'use strict';

  document.addEventListener('DOMContentLoaded', () => {
    const radios    = document.querySelectorAll('input[name="c-movimiento"]');
    const detalleEl = document.getElementById('js-index-movimiento-detalle');
    const detalleInput = document.getElementById('c-mov-detalle');

    if (!radios.length || !detalleEl) return;

    radios.forEach(radio => {
      radio.addEventListener('change', () => {
        const esSi = radio.value === 'si' && radio.checked;
        detalleEl.hidden = !esSi;
        if (!esSi && detalleInput) detalleInput.value = '';
      });
    });
  });
})();
